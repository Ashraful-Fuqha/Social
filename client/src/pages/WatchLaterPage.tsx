...
import { useRemoveFromWatchLaterMutation } from '@/store/api/mutations';
import { useGetUserWatchLaterQuery, useGetVideosByIdsQuery } from '@/store/api/queries'
...
function WatchLaterPage() {
    const { watchLater } = useAuthStore()
    const { isLoading, isError, error, refetch } = useGetUserWatchLaterQuery() 
    const {data: videos} = useGetVideosByIdsQuery(watchLater)
    ...
    const removeVideoMutation = useRemoveFromWatchLaterMutation()
  ...
  return (
    <div>
      <h1 className='text-2xl font-bold mb-6 text-gray-800 dark:text-white'>Your Watch Later Videos</h1>
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {videos && videos.length > 0 ? (
          ...
          <div>
            <div className="text-center text-slate-500">There are no videos. add some videos</div>
          </div>
        )}
